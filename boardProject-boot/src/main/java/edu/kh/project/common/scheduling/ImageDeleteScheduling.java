...
public class ImageDeleteScheduling {
	...
	//@Scheduled(fixedDelay = 5000) // 이전 작업이 시작된 후 5초
	...
	@Scheduled(cron = "0,15 * * * * *") // 시계 초 단위가 0, 30인 경우 수행 (테스트 용)
	public void scheduling() {
		...
		// 배열 내용 복사 (깊은 복사)
		// System.arraycopy(복사할 배열, 몇번인덱스부터 복사할지, 새로운배열, 새로운배열의몇번부터넣을지인덱스, 복사를어디까지할건지)
		System.arraycopy(boardArr, 0, imageArr, 0, boardArr.length);
		System.arraycopy(boardArr, 0, imageArr, boardArr.length, memberArr.length);
